...
export type ArgumentType =
  | 'none' // No argument allowed (e.g., *page, *quit)
  | 'text' // Free text (e.g., *button: Click me)
  | 'url' // URL expected (e.g., *image:, *audio:, *video:)
  | 'number' // Numeric value (e.g., *repeat: 3)
  | 'percent' // Percentage (e.g., *progress: 50%)
  | 'duration' // Duration expression (e.g., *wait: 5.seconds)
  | 'expression' // Any expression (e.g., *if: x > 5)
  | 'label' // Label name (e.g., *goto: myLabel)
  | 'variable' // Variable name (e.g., *set: myVar)
  | 'program-name' // Program name (e.g., *program: My Program)
  | 'service-name' // Service name (e.g., *service: My API)
  | 'event-name' // Event name (e.g., *trigger: myEvent)
  | 'iteration' // For loop iteration (e.g., *for: item in collection)
  | 'enum'; // One of specific values

export type SubKeywordValueType =
  | 'none' // No value (flag-style, e.g., *shuffle)
  | 'text' // Free text
  | 'url' // URL value
  ...
  | 'enum' // One of specific values
  | 'collection' // Collection/array value
  | 'association'; // Association/object value
 ...
 */
export function getKeywordSpec(keyword: string): KeywordSpec | undefined {
 ...
 */
export function getRequiredSubKeywords(keyword: string): string[] {
 ...
 */
export function getValidSubKeywords(keyword: string): string[] {
  const spec = getKeywordSpec(keyword);
  if (!spec?.subKeywords) return [];
 ...
 */
export function isValidSubKeyword(
  parentKeyword: string,
  subKeyword: string
): boolean {
  const spec = getKeywordSpec(parentKeyword);
  if (!spec?.subKeywords) return false;
 ...
 */
export function getSubKeywordEnumValues(
  parentKeyword: string,
  subKeyword: string
): string[] | undefined {
  const spec = getKeywordSpec(parentKeyword);
  if (!spec?.subKeywords) return undefined;
